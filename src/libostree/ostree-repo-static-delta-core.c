                                ...
                                GVariant **out_part, GCancellable *cancellable, GError **error)
{
   ...
   * per-object margin on top of usize so legitimate parts aren't
   * rejected; see OSTREE_STATIC_DELTA_PART_USIZE_MARGIN_BYTES.
   */
  guint64 max_part_usize = OSTREE_STATIC_DELTA_PART_MAX_USIZE_BYTES;
  if (expected_usize > 0)
    {
      guint64 margin
          = OSTREE_STATIC_DELTA_PART_USIZE_MARGIN_BYTES
            + ((guint64)expected_n_objects * OSTREE_STATIC_DELTA_PART_USIZE_MARGIN_BYTES);
