  ...
  while (TRUE)
    {
      ...
      if (!openat_ignore_enoent (dfd_iter.fd, rev, &ret_fd, error))
        goto out;
                        ...
                        GError       **error)
{
  ...
  if (!glnx_dirfd_iterator_init_at (base_dfd, base_path->str, FALSE, &dfd_iter, error))
    goto out;
      ...
      if (dent->d_type == DT_DIR)
        {
          ...
          if (!enumerate_refs_recurse (repo, remote, base_fd, base_path,
        ...
        {
          if (!add_ref_to_set (remote, base_fd, base_path->str, refs,
                               cancellable, error))
            goto out;
        }
                       ...
                       GError          **error)
{
  ...
  if (refspec_prefix)
  ...
  else
    {
      ...
      if (!enumerate_refs_recurse (self, NULL, refs_heads_dfd, base_path, refs_heads_dfd,
                                   ret_all_refs,
                                   cancellable, error))
        goto out;
      ...
      while (TRUE)
        {
          ...
          if (!enumerate_refs_recurse (self, dent->d_name, dfd_iter.fd, base_path, dfd_iter.fd,
                                       ret_all_refs,
                                       cancellable, error))
            goto out;
        }
