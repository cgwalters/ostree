                             ...
                             GCancellable *cancellable, GError **error)
{
  ...
  if (!sysroot->boot_is_vfat)
    {
      if (!create_bootself_link (sysroot, cancellable, error))
        return FALSE;
    }
                            ...
                            GCancellable *cancellable, GError **error)
{
  ...
  /* This allows us to support both /boot on a seperate filesystem to / as well
   * as on the same filesystem; but we skip this when /boot is the ESP.
   */
  if (!sysroot->boot_is_vfat)
    {
      if (TEMP_FAILURE_RETRY (symlinkat (".", sysroot->sysroot_fd, "boot/boot")) < 0 && errno != EEXIST)
        return glnx_throw_errno_prefix (error, "symlinkat");
    }
...
/* Update the /boot/loader data to point to /boot/loader.$new_bootversion.
 * If loader_link is TRUE, we assume that /boot/loader is a symlink. Otherwise
  ...
  if (!self->boot_is_vfat)
    {
      if (!glnx_fstatat_allow_noent (self->sysroot_fd, "boot/loader", &stbuf, AT_SYMLINK_NOFOLLOW,
                                     error))
        return FALSE;
      if (!create_bootself_link (self, cancellable, error))
        return FALSE;
    }
